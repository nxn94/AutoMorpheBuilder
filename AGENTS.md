...
- **`java.lang.NoSuchMethodError: ...BytecodePatchBuilder.extendWithAll(Supplier)` during `list-versions`** — the patches `.mpp` jar was compiled against a newer morphe-patcher than the cached `morphe-desktop.jar` ships. Patches built against newer patcher APIs (e.g. hoo-dles/morphe-patches uses `extendWithAll` since 2026-07-27) crash every patch's `<clinit>` on the older CLI. Two root causes, both fixed: (1) the morphe-tools cache restore-keys now scope strictly to the current `cli-version`, so a cache miss on the exact key can't silently restore an older jar (an over-broad `morphe-tools-` fallback used to do that); (2) `compat-probe.js` runs `list-versions` per app right after the jar download, classifies stderr for `NoSuchMethodError` / `NoClassDefFoundError` / `IncompatibleClassChangeError` / `VerifyError` / `LinkageError`, and emits an actionable `::error::` ("bump cli or pin each affected app to an older patch tag") instead of waiting 30 minutes for the matrix to surface the same crash.
