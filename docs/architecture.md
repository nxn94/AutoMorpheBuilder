...
3. **Alias resolution.** When `KEY_ALIAS` is unset, `--keystore-entry-alias` is omitted and morphe-desktop picks the keystore's first alias. When `KEY_PASSWORD` equals `KEYSTORE_PASSWORD`, `--keystore-entry-password` is omitted (morphe-desktop reuses the store password). Identical-password keystores stay a single env var.
...
Required secrets: `KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`. Optional: `KEY_ALIAS` (defaults to first alias — relies on morphe-desktop picking the first alias when `--keystore-entry-alias` is omitted; verify this against your morphe-desktop version if the build fails with "alias not found"), `KEY_PASSWORD` (only if the key password differs from the keystore password).
